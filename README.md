...
Методы класса: 
`getProducts(): Promise<IProductResponse>` - выполняет GET запрос к эндпоинту /product/ и возвращает промис с массивом товаров.
...
Поля класса:
`protected titleElement` - наименование товара.
`protected priceElement` - цена товара.
...
#### Класс CardPreview
Класс отвечает за отрисовку делальной информации о карточке товара в модальном окне при клике на нее в каталоге. Класс содержит сеттеры category - устанавливает категорию, image - устанавливает картинку и description - устанавливает описание, а так же метод buttonTextToggle, который устанавливает текст кнопки и ее активность. Наследуется от класса Card.

Конструктор: `constructor(container: HTMLElement, protected events: IEvents)` - принимает на вход контейнер карточки и брокер событий.
...
`buttonTextToggle(value: string, boolean: boolean)` - устанавливает текст и активность кнопки.
...
#### Класс Form<T>
Класс отвечает за базовую отрисовку формы оформления заказа. Является родителем для остальных форм при оформлении заказа. Класс имеет сеттеры errorForm - для утановки текст ошибок и toggleButtonForm для переключения состояния кнопки отправки формы, так же метод emitChange для обработки событий изменения формы. Наследуется от класса Component<T>.

Конструктор: `constructor(protected events: IEvents, container: HTMLFormElement)` - принимает на вход брокер событий и  контейнер формы.
...
`protected emitChange(field: keyof T, value: string)` - обработчик собитий изменения формы.
...
order:submit - Отправляется, когда пользователь нажимает кнопку далее в форме с данными доставки и оплаты.
contacts:submit - Отправляется, когда пользователь нажимает кнопку далее в форме с контактами.
form:change - Отправляется, когда происходят изменения в форме.
catalog:change - Отправляется, когда происходят изменения в каталоге. 
catalog:selectedProductChange - Отправляется, когда происходят изменения с выбранным продуктом.
'preview:click' - Отправляется, когда пользователь кликнул на кнопку Купить/Убрать из корзины.
customer:change - Отправляется, когда происходят изменения данных покупателя.
shoppingCart:change - Отправляется при добавлении/удалении товаров из корзины.
product:selected - Отправляется, когда пользователь выбрал продукт.
catalog:selectedProductChange - Отправляется, когда пользователь выбрал товар из галереи.


### Презентер

main.ts выполняет роль презентера в архитектуре приложения. Он связывает модели, представления и систему событий, обеспечивая логику взаимодействия между ними.

Основные задачи презентера:

Инициализация моделей, представлений и событий.
Подписка на ключевые события пользовательского интерфейса и изменение данных.
Обработка событий для обновления состояния моделей и синхронизация UI.
Валидация данных пользователя через модели и управление отображением ошибок.
Управление открытием и закрытием модальных окон и форм.
Контроль состояния корзины покупок, обновление счетчиков и списков товаров.
Вызов API для загрузки каталога продуктов и отправки заказов.
Обеспечение взаимодействия между компонентами приложения без прямой связи между ними.

Взаимодействия:

Использует модели Catalog, Customer, ShoppingCart для управления данными.
Обновляет UI-компоненты (Basket, Card, CardBasket, CardCatalog, CardPreview, Form, FormContacts, FormOrder, Gallery, Header, Modal, SuccessOrder) на основе событий и данных моделей.
Отслеживает пользовательские действия через EventEmitter, делегируя обработку событий и обновление интерфейса.
Производит загрузку и отправку данных через Communication и Api.
